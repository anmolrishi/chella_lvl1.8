...
import { FaPhoneAlt, FaClock, FaDollarSign, FaSmile } from 'react-icons/fa';
...
export default function Analytics({ userId }: AnalyticsProps) {
  ...
  const totalCalls = Object.keys(analytics).length;
  const totalDuration = Object.values(analytics).reduce(
    (sum, call) => sum + call.duration_ms,
    0
  );
  const averageDuration =
    totalCalls > 0 ? totalDuration / totalCalls / 1000 : 0;
  const totalCost = Object.values(analytics).reduce(
    (sum, call) => sum + call.call_cost.combined_cost,
    0
  );

  // Prepare data for charts
  const callDurationData = Object.entries(analytics).map(([callId, data]) => ({
    name: callId.slice(0, 8),
    duration: parseFloat((data.duration_ms / 1000).toFixed(2)),
  }));

  const sentimentData = Object.values(analytics).reduce((acc, call) => {
  ...
  const handleRowClick = (callId: string) => {
    setSelectedCall({ callId, data: analytics[callId] });
    onOpen();
  };
      ...
      >
        <VStack spacing={8} align="stretch">
          <Heading as="h1" size="xl" textAlign="center" color="blue.800">
            {restaurantName}'s Call Analytics
          </Heading>
          ...
          <HStack spacing={6}>
            ...
            <Box
              ...
              textAlign="center"
            >
              <FaDollarSign size={40} color="#38A169" />
              <Stat mt={2}>
                <StatLabel fontSize="lg" mt={2}>
                  Total Cost
                </StatLabel>
                <StatNumber fontSize="3xl">${totalCost.toFixed(2)}</StatNumber>
              </Stat>
            </Box>
            <Box
              flex="1"
          ...
          <Box bg="white" p={6} borderRadius="md" boxShadow="lg">
            ...
            <TableContainer>
              <Table variant="simple">
                ...
                <Tbody>
                  {Object.entries(analytics).map(([callId, data]) => (
                    <Tr
                      key={callId}
